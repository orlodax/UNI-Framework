...
{
    [ClassInfo(SQLName = "outboundddts")]
    public class OutboundDDT : BaseModel
    {
        [ValueInfo(SQLName = "number", IsDisplayProperty = true, IsMainNumeration = true)] public double Number { get; set; }
        ...
        [ValueInfo()] public List<OutboundDDTRow> OutboundDDTRows { get; set; }
        ...
        public OutboundDDT()
        {
            ...
            VatPrice = 0;
            GrossPrice = 0;
            foreach (var row in OutboundDDTRows ?? new List<OutboundDDTRow>())
            {
                //row.Loaded();
                NetPrice += row.UnitNetPrice * row.NominalQuantity;
                VatPrice += row.UnitVat * row.NominalQuantity;
